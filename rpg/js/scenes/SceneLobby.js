export class SceneLobby extends Phaser.Scene {
    ...
    create() {
        ...
        const head = this.add.circle(0, -22, 8, this.registry.get('playerColor') || 0x00e8ff).setStrokeStyle(2, 0xffffff, 1);
        ...
        const actions = ['WORLD MAP', 'CREATE AGENT', 'HELP'];
        actions.forEach((act, i) => {
            const x = 140 + (i * 140);
            const btn = this.add.rectangle(x, h - 40, 120, 36, 0x000080).setStrokeStyle(2, 0xffffff, 0.8).setInteractive({ cursor: 'pointer' });
            this.add.text(x, h - 40, act, { fontFamily: 'VT323', fontSize: '18px', color: '#fff' }).setOrigin(0.5);
            btn.on('pointerdown', () => this.cameras.main.flash(400, 0, 232, 255));
        });
    ...
    addHUD(w, h) {
        this.logText = this.add.text(20, 20, '', { fontFamily: 'VT323', fontSize: '18px', color: '#00ff88', alpha: 0.6 });
    }
